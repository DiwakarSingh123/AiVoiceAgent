 ...
 const navigate=useNavigate();
    ...
    <header className="w-full fixed top-0 z-50 bg-black/20 backdrop-blur-xl border-b border-white/10">
      ...
      <div className="w-[80%] mx-auto py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {
            user==null &&
            <button onClick={()=>navigate('/signin')} className="hidden md:block px-5 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-200 hover:bg-white/10 transition">
            Sign In
           </button>
          }

          <button onClick={()=>{user ? user.assistantImage && user.assistantName ? navigate("/assistant") : navigate('/customize'): navigate('/signup')}} className="hidden md:block px-5 py-2 rounded-lg font-semibold border-2 border-sky-300 
  ...
  <button onClick={() => document.getElementById("demo").scrollIntoView({ behavior: "smooth" })}>
    Demo
            ...
            <button className="px-5 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-200 hover:bg-white/10 transition">
              Sign In
            </button>

            <button className="px-5 py-2 rounded-lg font-semibold border-2 border-sky-300 
                       text-transparent bg-clip-text bg-[linear-gradient(90deg,#5DF5FF,#6E9BFF,#A06CFF)]
                       shadow-md hover:shadow-xl active:scale-95 transition">
              Get Started
            </button>
